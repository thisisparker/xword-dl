...
class NewYorkerCrypticDownloader(AmuseLabsDownloader):
    command = 'tnyc'
    outlet = 'New Yorker Cryptic'
    outlet_prefix = 'New Yorker Cryptic'

    def __init__(self, **kwargs):
        super().__init__(**kwargs)

        self.url_from_id = 'https://cdn3.amuselabs.com/tny/crossword?id={puzzle_id}&set=tny-new-cryptic'

    @staticmethod
    def matches_url(url_components):
        return ('newyorker.com' in url_components.netloc and '/puzzles-and-games-dept/cryptic-crossword' in url_components.path)
    ...
    def find_latest(self):
        index_url = "https://www.newyorker.com/puzzles-and-games-dept/cryptic-crossword"
        index_res = requests.get(index_url)
        index_soup = BeautifulSoup(index_res.text, "html.parser")
    ...
    def parse_xword(self, xword_data):
        puzzle = super().parse_xword_to_pypuz(xword_data)

        if '<' in puzzle.metadata.title:
            puzzle.metadata.title = puzzle.metadata.title.split('<')[0]

        return puzzle

    def pick_filename(self, puzzle, **kwargs):
        try:
            supra, main = puzzle.metadata.title.split(':', 1)
            if supra == 'The Cryptic Crossword' and dateparser.parse(main):
                title = ''
            else:
                title = main.strip()
        except XWordDLException:
